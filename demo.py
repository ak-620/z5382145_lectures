...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    dat = os.path.join(DATDIR, tic)
    dat1 = dat + '_prc.dat'
    data = open(dat1, 'r').readlines()
...
def line_to_dict(line):
    ...
    # line1 = line.replace(" ", "")
    line1 = line.strip()
    lines = int(len(COLUMNS))
    dic = {}
    index = 14
    for i in range(0, lines):

        if i > 0:
            dic[COLUMNS[i]] = line1[index: index + COLWIDTHS.get(COLUMNS[i])]
            index += COLWIDTHS.get(COLUMNS[i])
            print(index)
        else:
            dic[COLUMNS[i]] = line1[:COLWIDTHS.get(COLUMNS[i])]
